...
describe('fetchPagesEnvState', () => {
  it('reads the production env_vars and config hash', async () => {
    ...
    const state = await fetchPagesEnvState({ project: 'share-text', accountId: 'acct', token: 'tok', fetchImpl })
    expect(state).toEqual({ envVars: { A: PLAIN('1') }, configHash: 'h1' })
    expect(calls[0].url).toBe('https://api.cloudflare.com/client/v4/accounts/acct/pages/projects/share-text')
  })
