...
- The CSV grid model (`src/lib/components/use-grid-model.svelte.ts`) treats incoming `value` as a self-echo when it differs from the last committed matrix only by trailing all-empty rows (the CSV round-trip through `parseCsv` drops those rows). Keep that reconciliation in place so rows appended empty by the grid (e.g. ArrowDown on the last cell) survive the preview content feedback; a genuine external change with different data rows still rebuilds the grid.
